...
class SkippedValidationServiceImplTest {
    ...
    void createSkippedValidations_WithNoClausesMatchingErrorCodes_CallsCreateWithEmptyList() {
        ...
        Mockito.when(clauseCache.getByErrorCode(Mockito.anyInt())).thenReturn(Optional.empty());
    ...
    void createSkippedValidations_WithClausesMatchingErrorCodes_CreatesSkippedValidationsInRepository() {
        ...
        Mockito.when(clauseCache.getByErrorCode(1)).thenReturn(Optional.of(clause1));
        Mockito.when(clauseCache.getByErrorCode(2)).thenReturn(Optional.of(clause2));
