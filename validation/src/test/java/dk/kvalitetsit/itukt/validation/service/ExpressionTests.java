...
public class ExpressionTests {
    ...
    static Stream<Arguments> allErrorNumberConditionCombinations() {
        return Stream.of(Operator.values())
                .map(ExpressionTests::toErrorAgeConditionExpression)
                .map(Arguments::of);
    }

    static ExpressionAndExpectedError toErrorAgeConditionExpression(Operator op) {
    ...
    void validate_errorShouldBeReturned4() {
        ...
        var result = combined.validates(validationInput);
        assertErrorMessage("alder skal være 39, 38, 37 eller 36", result);
    }
