...
sealed public interface ValidationError permits AndError, ConditionError, ExistingDrugMedicationError, OrError {
    default String toErrorString() {
        return switch (this) {
            case AndError andError -> andError.toErrorString();
            case OrError(var e1, var e2) -> e1.toErrorString() + " eller " + e2.toErrorString();
            case ConditionError conditionError -> conditionError.toErrorString();
            case ExistingDrugMedicationError existingDrugMedicationError -> existingDrugMedicationError.toErrorString();
        };
    }
